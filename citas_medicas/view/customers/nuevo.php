...
if (isset($_POST['agregar'])) {
	...
	try {
		...
		VALUES (1, :nombrep, :apellidop, :seguro, :tele,:sexo,:email,:clave,:cargo,:estado)");
		//instrucción if-else en la ejecución de nuestra declaración preparada
		$_SESSION['message'] = ($stmt->execute(array(
			':dnipa' => $_POST['dnipa'], ':nombrep' => $_POST['nombrep'], ':apellidop' => $_POST['apellidop'],
			':seguro' => $_POST['seguro'], ':tele' => $_POST['tele'], ':sexo' => $_POST['sexo'], ':email' => $_POST['email'], ':clave' => MD5($_POST['clave']),
			':cargo' => $_POST['cargo'], ':estado' => $_POST['estado']
		))) ? 'Paciente guardado correctamente' : 'Algo salió mal. No se puede agregar miembro';
